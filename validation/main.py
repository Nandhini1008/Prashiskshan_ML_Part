...
class CompanyLegitimacyValidator:
    ...
    async def validate_mca_async(self) -> Dict[str, Any]:
        """Run MCA validation asynchronously in headless mode"""
        ...
        try:
            ...
            def run_mca_validation():
                ...
                try:
                    # Always run in headless mode
                    agent = MCAAutomationAgent(headless=True)
                    result = agent.fetch_company_details(self.company_name, self.cin_number)
                    return result
                finally:
                    if agent:
                        agent.close()
